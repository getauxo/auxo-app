...
//   앱이 조용히 받아 두고, **다음에 앱을 켤 때 설치**한다. 사용자가 할 일은 없다.
//   쓰는 동안에는 앱이 닫히지 않고, 바꾸는 동안에는 설치 진행 화면이 보인다.
//
...
function setupAutoUpdate() {
  ...
  //   ★원칙: **앱 창이 열린 뒤로는 업데이트가 앱을 닫지 않는다.**
...
app.whenReady().then(async () => {
  ...
  if (!isSmokeMode) {
    // 받아둔 새 버전이 있으면 **기다리지 않고 바로** 간다 — 사용자를 5초 쓰게 해놓고
    //   갑자기 "바꾸는 중"으로 끊으면 그게 더 놀랍다. 켜자마자 안내하고 끝내는 게 낫다.
    const 바꿀것 = 받아둔새버전();
    if (바꿀것) { cleanupUpdaterCache(); setupAutoUpdate(); }
    else setTimeout(() => { cleanupUpdaterCache(); setupAutoUpdate(); }, 5000);
  }
