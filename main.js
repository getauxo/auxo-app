  ...
  return await new Promise((resolve) => {
    ...
    const 확인 = () => {
      try {
        if (brainMode === 'claude-subscription') {
          const s = brainClaude.authStatus();
          if (s && s.loggedIn) return { ok: true, loggedIn: true, account: { email: s.email || '', plan: s.subscriptionType || '' } };
        } else if (brainCodex.loginStatus().loggedIn) return { ok: true, loggedIn: true, account: null };
      } catch (_) {}
      return null;
    };
