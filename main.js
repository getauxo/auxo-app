...
//   **앱을 켤 때** 스플래시 단계에서 확인·받기·설치를 끝내고, 그다음 앱이 열린다.
//   진행바가 보이므로 사용자는 무슨 일인지 안다. 따로 할 일은 없다.
//   ★앱 화면이 열린 뒤로는 업데이트가 아무것도 하지 않는다 — 받지도, 닫지도 않는다.
//     (드물게 앱을 켜둔 채로 새 판이 나오면 그때만 받아두고 다음에 켤 때 바꾼다)
//
...
let _대화시작 = false;
// 스플래시가 걷히고 **사용자에게 앱 화면이 보이기 시작했나.** 업데이트 설치의 경계선이다.
//   이 값이 true 가 되는 순간부터 업데이트는 아무것도 하지 않는다 — 받지도, 닫지도 않는다.
//   렌더러가 스플래시를 걷을 때 알려준다(hideSplash → update:window-open).
let _창열림 = false;
...
function setupAutoUpdate() {
  ...
  // ★끌 때 설치하지 않는다 — **앱을 켤 때** 설치한다.
  //   왜: 끄고 난 뒤 뒤에서 설치하면, 그 13초 동안 Auxo.exe 가 없어 바로가기가 깨진 것처럼 보인다.
  //   그때 아이콘을 누르면 Windows 가 "이 바로 가기를 삭제하시겠습니까?" 를 묻고,
  //   「예」를 누르면 바로가기가 진짜 지워진다(실사용자 실측 2026-08-16, 공백 4.6초 재현).
  ...
  // 언제 설치하나 — **켜는 그 자리에서, 창이 열리기 전에.**
  //   ★정한 그림(2026-08-16): "앱 사용할 때는 그냥 있고, 앱을 종료했다가 다시 실행할 때
  //     자동 업데이트되면서 그때 진행바가 보이는 상태였다가 완료되면 앱이 실행되는 것".
  //     즉 **받기도 설치도 스플래시 단계에서** 끝난다. 한 번 켜면 끝이다.
  //
  //   ⚠️ 한때 이걸 절반만 구현했다(2026-08-16 22:04 ~ 2026-08-21). 설치만 다음 실행으로 미루고
  //     **받기는 앱을 쓰는 중에** 했다. 그래서 사용자는 앱을 **두 번 켜야** 새 버전이 됐고,
  //     지난번에 받아둔 걸 모르니 다음에 켤 때 "왜 갑자기 뭐가 돌지?" 가 됐다.
  //     승인받은 그림과 다른 것을 만들어놓고 오래 알아채지 못했다.
  //
  //   대가: 업데이트가 있는 날은 시작이 20초쯤 늦어진다. 차등이 살아 있어 그 정도다(실측 19초·31블록).
  //     그 대신 **앱이 열린 뒤로는 업데이트가 아무것도 하지 않는다** — 쓰는 중에 받지도, 닫지도 않는다.
  //
  //   창이 이미 열렸는지는 _창열림 으로 본다. 스플래시 단계면 아직 false 다.
  //   대화시작됨() 은 그 위의 안전장치 — 텔레그램·디스코드가 붙어 있으면 손대지 않는다.
  ...
  autoUpdater.on('download-progress', (p) => {
    // 스플래시 단계에서 받는 중이면 곧 그 자리에서 바뀐다. 창이 이미 열렸으면 다음 실행으로 미룬다.
    const 뒤 = _창열림 ? ' — 다음에 켤 때 바뀝니다' : ' — 곧 새 버전으로 바뀝니다';
    알림('downloading', `새 버전을 받는 중이에요 ${Math.round((p && p.percent) || 0)}%${뒤}`);
  });
  autoUpdater.on('update-downloaded', (i) => {
    const v = (i && i.version) || '';
    if (!_창열림 && !대화시작됨()) {
      // 아직 스플래시다 → **지금이 바꿀 자리다.** 사용자가 보고 있는 화면을 뺏지 않는다.
      알림('installing', `새 버전 ${v} 으로 바꾸는 중이에요`, v);
      지금설치(v);
    } else {
      // 창이 이미 열렸거나 다른 채널이 붙어 있다 → **절대 건드리지 않는다.**
...
function 지금설치(version) {
  try {
    if (mainWindow && !mainWindow.isDestroyed()) {
      mainWindow.webContents.send('update:installing', { version });
...
/** 켤 때 바로 바꿀 것이 있나 — 창을 그리기 전에 알아야 스플래시를 그대로 둘 수 있다. */
function 받아둔새버전() {
  try {
    const base = process.env.LOCALAPPDATA; if (!base) return null;
    const info = path.join(base, 'auxo-updater', 'pending', 'update-info.json');
    if (!fs.existsSync(info)) return null;
    const name = (JSON.parse(fs.readFileSync(info, 'utf8')) || {}).fileName || '';
    const m = /(\d+)\.(\d+)\.(\d+)/.exec(name); if (!m) return null;
    const 받아둔 = [+m[1], +m[2], +m[3]];
    const 지금 = String(APP_VERSION).split('.').map(Number);
    for (let i = 0; i < 3; i++) {
      if ((받아둔[i] || 0) > (지금[i] || 0)) return m[0];   // 현재보다 높다 = 바꿀 것이 있다
      if ((받아둔[i] || 0) < (지금[i] || 0)) return null;
    }
    return null;
  } catch (_) { return null; }
}
ipcMain.handle('update:pending', () => ({ version: 받아둔새버전() }));
...
app.whenReady().then(async () => {
  ...
  // ★자동 업데이트는 **바로** 시작한다 — 스플래시가 걷히기 전에 끝내기 위해서다.
  //   정한 그림(2026-08-16): "앱을 종료했다가 다시 실행할 때 자동 업데이트되면서
  //   그때 진행바가 보이는 상태였다가 완료되면 앱이 실행되는 것".
  //   전엔 5초를 기다렸다가 시작해서, 그 사이 스플래시가 걷히고 **앱을 쓰는 중에 받았다.**
  //   그러면 이번 실행에서는 못 바꾸고 다음에 켤 때로 미뤄져 **두 번 켜야** 새 버전이 됐다.
  //   지금은 켜자마자 확인하고, 스플래시 단계에서 받고 그 자리에서 설치한다.
  //   ※ 늦게 시작할 이유가 없다 — 확인·다운로드는 뒤에서 돌아 시동을 막지 않는다.
  //     업데이트가 없으면(대부분의 실행) 아무 일도 안 일어나고 스플래시는 평소대로 걷힌다.
