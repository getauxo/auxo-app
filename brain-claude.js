 ...
 *
 * @param opts.toolsOutsideSandbox 우리 도구가 **CLI 샌드박스 밖에서** 돈다는 사실(구독 두뇌=true).
 *   ★없으면 두뇌가 **자기 자물쇠를 우리 도구의 자물쇠로 착각**해 아예 안 부르고 "차단됐다"고 답한다.
 *   codex 는 도구 턴에 `-s workspace-write` + 빈 임시 cwd 로 돈다(brain-codex.js) → **자기 손은 진짜로 막혀 있다.**
 *   그런데 make_dir 등은 별도 프로세스(auxo-mcp-tools)에서 **우리 허용 체계로** 돈다 — 그 자물쇠와 **무관하다.**
 *   실측(2026-08-21, codex, 판정은 DB 장부로): 이 한 줄 **없이 0/8**, **붙이면 8/8** 호출.
 *     없을 때 답 = *"시도했지만 현재 도구 권한 정책에서 차단됐습니다"* (부른 적 없음 = 거짓).
 *     붙였을 때 = make_dir 호출 → 막힘 → **사용자에게 허용 요청**(원래 있어야 할 흐름).
 *   ※ 이건 위 "더 세게 쓰기"(실패)와 **종류가 다르다.** 훈계가 아니라 **틀린 사실을 바로잡는 것**이다.
 *     같은 성격인 toolsAreLive 가 0/4 → 4/4 로 통했던 것과 같은 자리.
 *   ⚠️ claude 구독은 **미검증**이다(네이티브 파일 도구를 --disallowedTools 로 막아 구조는 같다).
 *      REST 두뇌엔 붙이지 마라 — 거긴 이 샌드박스가 없어 이 말이 거짓이 된다.
 */
function buildSystemPrompt(agentName, persona, memory = {}, layer2 = {}, availableTools = AGENT_TOOLS, skills = [], opts = {}) {
  ...
  const speechLines = [];
  // ★사용자가 **말로 정한 말투가 있으면 그게 먼저다.** 미러링은 그때 안 본다.
  //   2026-08-20: 전엔 미러링 문장이 앞에 있고 "명시하면 최우선"이 뒤에 붙어 있었다.
  //   사용자가 **반말로 말하면서 "존칭 써줘"라고 한 경우** 두 줄이 정면으로 부딪친다 —
  //   실제로 8턴은 존댓말을 지키다 한 턴에서 반말로 무너졌다(정직 계층 되돌림 턴).
  //   되돌림은 대화 이력을 안 보내므로 "존칭 써줘"가 안 보였고, 남은 건 미러링뿐이었다.
  //   → 지시를 **저장해 두고(userSpeech) 맨 앞에** 놓는다. 이력이 빠지는 경로에서도 살아남는다.
  if (layer2.userSpeech === 'formal') {
    speechLines.push('★사용자가 **존댓말로 해 달라고 직접 말했다.** 사용자가 너에게 반말로 말하더라도 너는 **항상 정중하고 따뜻한 존댓말**로 답해. 이게 다른 어떤 말투 규칙보다 우선한다.');
  } else if (layer2.userSpeech === 'casual') {
    speechLines.push('★사용자가 **반말로 해 달라고 직접 말했다.** 사용자가 너에게 존댓말로 말하더라도 너는 **편하고 다정한 반말**로 답해. 이게 다른 어떤 말투 규칙보다 우선한다.');
  } else {
  ...
  if (tools.length === 0) {
    ...
    // ★두뇌가 **자기 샌드박스**와 **우리 도구**를 뒤섞지 않게 사실을 못박는다(위 opts 주석의 실측 0/8 → 8/8).
    const 밖에서돈다 = opts && opts.toolsOutsideSandbox
      ? '★위 도구는 네 프로세스 **밖에서** 돌아가고 **자체 허용 체계**를 쓴다. 네 셸·파일 권한이 막혀 있어도 이건 별개야. '
        + '"내 실행 정책이 막혀 있으니 못 한다"고 **미리 판단하지 마** — 실제로 불러보고, 막히면 그때 사용자에게 허용 요청이 뜬다. '
        + '**부르지 않으면 그 요청조차 안 생긴다.** '
      : '';
    capText = `지금 너가 쓸 수 있는 도구${살아있음}: ${tools.join(', ')}. `
      ...
      // ★**목록에 없을 때 어디로 갈지**를 알려준다. 이게 없으면 곧장 "못 한다"로 끝난다.
      //   실측(2026-08-21): "폴더 이름 바꿔줘" 에 그런 도구가 없자 **셸로 건너뛰었고**,
      //   셸이 막혀 있으니 아무것도 못 했다. 중간 단계(스킬·MCP 찾기)를 통째로 건너뛴 것이다.
      //   순서를 정해주면 한 칸씩 내려간다 — 못 한다는 말은 **맨 마지막**이다.
      + '**이 목록에 없는 일을 부탁받으면 순서대로 시도해.** '
      + '①위 도구로 되는지 본다 → ②안 되면 find_skill·find_mcp 로 찾아서(사용자 승인 후) 설치해 쓴다 '
      + '→ ③그래도 없으면 run_shell·run_code 로 직접 한다. '
      + '**"그럴 수단이 없다"는 셋 다 안 될 때만 하는 말이야.** 중간을 건너뛰고 못 한다고 하지 마. '
      + 밖에서돈다
