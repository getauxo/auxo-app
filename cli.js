...
async function showNoticeLine() {
  try {
    ...
    if (line) {
      ...
      // ★새 버전이 있으면 **받는 방법까지** 알려준다.
      //   앱은 알아서 최신이 되지만 CLI 는 스스로 바뀌지 않는다(electron-updater 를 쓰지 않는다).
      //   "나왔어요" 만 띄우면 터미널 사용자는 알고도 방법이 없다 — 앱을 켜라고 안내할 수도 없다.
      //   CLI 만 쓰는 사람이 있기 때문이다.
      console.log('  ' + c.dim('(공지 그만 받기: /notice off)') + '\n');
      if (n && n.latestVersion && notice.cmpVer(n.latestVersion, APP_VERSION) > 0) {
        return await 업데이트제안(n.latestVersion);   // true = 바꿨으니 여기서 끝낸다
      }
    }
...
/**
 * 새 버전으로 바꿔준다 — **대화를 시작하기 전에만.**
 *
 * 왜 여기서만 하나:
 *   터미널 사용자는 앱과 달리 우리가 창을 닫았다 열 수 없다. 대화 중에 소스가 바뀌면
 *   하던 말이 끊기고, 무엇이 일어났는지도 안 보인다. 그래서 **시작 시점 한 자리**로 고정한다.
 *
 * 왜 물어보나:
 *   앱은 조용히 바꿔도 되지만(창이 다시 열린다) 터미널은 갑자기 `npm install` 이 돌면
 *   사용자가 무슨 일인지 모른다. 엔터 한 번이면 되게 하되, 결정은 사용자가 한다.
 *
 * 실패하면 **원래대로 되돌린다.** 소스만 새것이고 의존성이 옛것이면 CLI 가 아예 안 뜬다 —
 *   자동 업데이트가 사용자를 더 나쁜 자리에 데려다 놓는 일은 없어야 한다.
 */
async function 업데이트제안(newVer) {
  const { execFileSync } = require('child_process');
  const 여기 = __dirname;
  const 조용히 = { cwd: 여기, encoding: 'utf8', stdio: ['ignore', 'pipe', 'pipe'] };

  // ① 자동으로 할 수 있는 자리인가 — 아니면 방법만 알려주고 물러난다
  if (!fs.existsSync(path.join(여기, '.git'))) {
    console.log('  ' + c.dim('   새 소스를 내려받아 덮어쓴 뒤 ') + c.cyan('npm install --omit=dev') + c.dim(' 해주세요.') + '\n');
    return false;
  }
  try { execFileSync('git', ['--version'], 조용히); }
  catch (_) {
    console.log('  ' + c.dim('   git 이 없어 자동으로 바꾸지 못했어요. 새 소스를 받아 덮어써 주세요.') + '\n');
    return false;
  }

  // ② 물어본다 (엔터 = 예)
  const 답 = (await ask('  ' + c.b(`v${newVer} 으로 지금 바꿀까요?`) + c.dim(' (Enter=예 / n=나중에) '))).trim().toLowerCase();
  if (답 === 'n' || 답 === 'no' || 답 === 'ㅜ') {
    console.log('  ' + c.dim('   나중에 바꾸시려면: ') + c.cyan('git pull && npm install --omit=dev') + '\n');
    return false;
  }

  // ③ 되돌아올 지점을 먼저 잡아둔다
  let 원래;
  try { 원래 = execFileSync('git', ['rev-parse', 'HEAD'], 조용히).trim(); }
  catch (_) { 원래 = null; }

  try {
    console.log('  ' + c.dim('   새 소스를 받는 중…'));
    // ★`--autostash` 를 쓰면 안 된다. 실측(2026-08-16): pull 은 되지만 치워둔 것을 되돌릴 때
    //   충돌이 나고, 그런데도 **종료코드가 0** 이라 성공으로 보인다. 사용자는 `UU` 충돌 상태에
    //   남는데, git 을 모르는 사람은 그 자리를 빠져나올 수 없다 — 자동화가 사람을 더 나쁜 곳에 둔다.
    // → 그래서 **치워두기만 하고 되돌리지 않는다.** 바뀐 것은 대개 npm 이 건드린
    //   package-lock.json 이라 되돌릴 이유도 없다. 정말 고친 게 있었다면 stash 에 그대로 있으니
    //   아래 안내대로 꺼내면 된다. 잃는 것은 없고, 충돌도 생기지 않는다.
    let 치웠나 = false;
    const 더러운가 = execFileSync('git', ['status', '--porcelain', '--untracked-files=no'], 조용히).trim();
    if (더러운가) {
      execFileSync('git', ['stash', 'push', '-m', 'auxo-update'], 조용히);
      치웠나 = true;
    }
    //   --ff-only: 갈라졌으면 억지로 합치지 않고 실패한다(사용자 것을 망치지 않는다)
    execFileSync('git', ['pull', '--ff-only'], 조용히);
    // pull 이 0 을 줘도 실제로 깨끗한지 **직접 확인한다** — 종료코드를 믿지 않는다
    const 충돌 = execFileSync('git', ['status', '--porcelain'], 조용히)
      .split('\n').filter((l) => /^(UU|AA|DD|AU|UA|DU|UD)/.test(l));
    if (충돌.length) throw new Error('합치는 중 충돌이 생겼습니다');
    if (치웠나) {
      console.log('  ' + c.dim('   ※ 고쳐두신 파일이 있어 따로 보관했어요. 꺼내기: ')
        + c.cyan('git stash pop'));
    }

    console.log('  ' + c.dim('   준비하는 중… (몇 분 걸릴 수 있어요)'));
    execFileSync('npm', ['install', '--omit=dev', '--no-audit', '--no-fund'],
      { ...조용히, shell: true, timeout: 15 * 60 * 1000 });

    console.log('\n  ' + c.green(`✓ v${newVer} 으로 바꿨어요.`) + c.dim(' 터미널을 다시 시작하면 새 버전으로 켜집니다.'));
    console.log('  ' + c.dim('   다시 시작: ') + c.cyan('npm run cli') + '\n');
    return true;
  } catch (e) {
    // ④ 실패 — 있던 자리로 돌려놓는다. "고장난 채로 두는 것" 이 제일 나쁘다.
    let 되돌림 = false;
    if (원래) {
      try { execFileSync('git', ['reset', '--hard', 원래], 조용히); 되돌림 = true; } catch (_) {}
    }
    const 왜 = String((e && (e.stderr || e.message)) || '').split('\n').find((s) => s.trim()) || '';
    console.log('\n  ' + c.red('바꾸지 못했어요.') + (왜 ? c.dim('  ' + 왜.slice(0, 120)) : ''));
    if (되돌림) console.log('  ' + c.dim('   쓰던 버전 그대로 되돌렸으니 그냥 쓰셔도 됩니다.'));
    console.log('  ' + c.dim('   직접 바꾸시려면: ') + c.cyan('git pull && npm install --omit=dev') + '\n');
    return false;
  }
}
...
async function main() {
  ...
  clear();
  banner();
  // 새 버전·공지 확인(옵트아웃이면 요청 안 함) — 앱과 동일 통로.
  //   여기서 새 버전으로 바꿨다면 **대화를 시작하지 않고 끝낸다.** 이미 낡은 코드가 메모리에 올라와 있고,
  //   그 상태로 대화를 이어가면 사용자는 "바꿨는데 그대로네" 를 겪는다.
  if (await showNoticeLine()) { rl.close(); return; }
